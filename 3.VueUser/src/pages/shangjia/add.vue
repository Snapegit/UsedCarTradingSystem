...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="商家照片" v-if="type!='cross' || (type=='cross' && !ro.shangjiazhaopian)" prop="shangjiazhaopian">
            <file-upload
            tip="点击上传商家照片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="商家照片" prop="shangjiazhaopian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shangjiazhaopian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
  ...
  export default {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='shangjiazhaopian'){
              this.ruleForm.shangjiazhaopian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      ...
      shangjiazhaopianUploadChange(fileUrls) {
          this.ruleForm.shangjiazhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
