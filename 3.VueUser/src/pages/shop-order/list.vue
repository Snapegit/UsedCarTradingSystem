...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button v-show="activeName == '未支付'" type="success" :style='{"margin":"2px auto",}' size="mini" @click="pay(scope.row)">余额支付</el-button>
          <el-button v-show="activeName == '未支付'" type="primary" :style='{"margin":"2px auto",}' size="mini" @click="alipay(scope.row)">沙箱支付</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
                        ...
                        if(item.type==1||item.type==3) {
                            ...
                            this.$http.post(table+'/update', this.user).then(res => {
                                ...
                                this.$http.post(`orders/update`, item).then(res => {
                                    if(item.tablename == `ershoushangpin`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`ershoushangpin/update`, good).then(res => {
                                        });
                                    }
                                    this.$message({
      ...
      // 沙箱支付
      alipay(row) {
		  this.$http.get('orders/alipay', {params: {tradeno: row.orderid, totalamount: row.total, subject:row.goodname}}).then(res => {
			if (res.data.code == 0) {
				  var newWin = window.open('_blank');
				  newWin.document.querySelector('body').innerHTML = res.data.data;
